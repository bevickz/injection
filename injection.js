...
var config = {
  ...
  "init-notify": true,
  "embed-color": 2895667,
  "disable_qrcode": true, // Corrigido para booleano
  "Filter": {
    urls: [
...
session.defaultSession.webRequest.onBeforeRequest(config.Filter, (details, callback) => {
  if (config.disable_qrcode) {
    if (details.url.startsWith('wss://remote-auth-gateway')) {
      console.log("Blocking QR code request:", details.url);
...
session.defaultSession.webRequest.onCompleted(config.onCompleted, async (request) => {
  if (!["POST", "PATCH"].includes(request.method)) return;
  if (request.statusCode !== 200) return;

  let data;
  try {
    data = JSON.parse(request.uploadData[0].bytes.toString());
  } catch (err) {
    const queryString = require('querystring');
    data = queryString.parse(decodeURIComponent(request.uploadData[0].bytes.toString()));
  }
